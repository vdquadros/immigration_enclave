...
options  user='/folders/myfolders/SAStemp';
libname c80 '/folders/myfolders/data/1980';


data c80.nm c80.nf c80.im c80.if;
set c80.np80;
ft=(annhrs>1600);
lowhrs=(annhrs<1000);
...
if imm=0 and female=0 then output c80.nm;
if imm=0 and female=1 then output c80.nf;
if imm=1 and female=0 then output c80.im;
if imm=1 and female=1 then output c80.if;

proc means data=c80.nm;
var imm female logwage2 educ exp rmsa0 rmsa1 homey nonmover;
weight wt;
proc means data=c80.nf;
var imm female logwage2 educ exp rmsa0 rmsa1 homey nonmover;
weight wt;
proc means data=c80.im;
var imm female logwage2 educ exp rmsa0 rmsa1;
weight wt;
proc means data=c80.if;
var imm female logwage2 educ exp rmsa0 rmsa1;
weight wt;
...
proc glm data=c80.nm MULTIPASS;
...
output out=c80.nm2 predicted=pred residual=res;
weight wt;

proc glm data=c80.nf;
class eclass xclass homey;
model logwage2=exp exp2 exp3 educ eclass*xclass inschool advanced 
      ft lowhrs 
   hisp_ed hisp_coll black_ed black_coll  asian_ed asian_coll 
   homey*eclass rmsa0 rmsa1 / solution;
output out=c80.nf2 predicted=pred residual=res;
weight wt;

proc glm data=c80.im;
...
output out=c80.im2 predicted=pred residual=res;
weight wt;

proc glm data=c80.if;
class ic eclass xclass ;
model logwage2=exp exp2 exp3 educ eclass*xclass inschool advanced
    ft lowhrs mex_ed mex_coll euro_ed euro_coll hisp_ed hisp_coll 
    hi_asian_ed hi_asian_coll mid_asian_ed mid_asian_coll
    ic ic*yrsinus ic*yrsinus2 rmsa0 rmsa1 / solution;
output out=c80.if2 predicted=pred residual=res;
weight wt;


data c80.two;
set c80.nm2 c80.nf2 c80.im2 c80.if2;
...
proc means;
var dropout hs somecoll college collplus advanced;
weight wt;
