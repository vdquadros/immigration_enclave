...
use year datanum serial hhwt statefip hhincome pernum perwt sex age birthyr race raced hispan hispand ///
	ancestr1 ancestr1d ancestr2 ancestr2d citizen yrsusa2 school educ educd ///
	gradeatt gradeattd empstat empstatd labforce occ classwkr classwkrd wkswork2 ///
	uhrswork inctot ftotinc incwage incbus00 incsupp incother incearn bpl ///
...
gen eclass = .
replace eclass = 1 if educd <= 61 
replace eclass = 2 if educd == 62 | educd == 63 | educd == 64
replace eclass = 3 if 65 <= educd & educd <= 100
replace eclass = 4 if 101 <= educd & educd <= 116
...
/* WAGES */
replace incwage=75000*1.43 if incwage == 75000 /*pareto fix*/
...
gen yrsinus2 = yrsinus*yrsinus if & imm == 1
